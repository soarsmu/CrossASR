...
CLASSIFIER_MODEL = "AdaBoost"

classifier_fpath = "model/" + CLASSIFIER_MODEL + ".sav"
classifier = joblib.load(classifier_fpath)
...
def generate_speech(text, timestamp):
    if (TTS == GOOGLE_TTS):
        ...
        if os.path.exists(linux_folder):
            ...
            os.system('ffmpeg -i /Users/mhilmiasyrofi/Documents/cross-asr/' + outfile +
                      ' -acodec pcm_s16le -ac 1 -ar 16000 /Users/mhilmiasyrofi/Documents/cross-asr/' + wavfile + ' -y')
...
def recognize_speech(timestamp):
    ...
    for sr in SR:
        if sr == ALEXA:
            ...
            transcriptions[PADDLEDEEPSPEECH] = paddledeepspeech_recognize(
                timestamp)
...
def calculate_recognition_error(text, transcriptions):
    ...
    for sr in SR:
        if sr == ALEXA:
            errors[ALEXA] = round(
                wer(text, preprocess_transcription(transcriptions[ALEXA])), 2)
        elif sr == DEEPSPEECH:
            errors[DEEPSPEECH] = round(
                wer(text, preprocess_transcription(transcriptions[DEEPSPEECH])), 2)
        elif sr == GCLOUD:
            errors[GCLOUD] = round(
                wer(text, preprocess_transcription(transcriptions[GCLOUD])), 2)
        elif sr == CHROMESPEECH:
            errors[CHROMESPEECH] = round(
                wer(text, preprocess_transcription(transcriptions[CHROMESPEECH])), 2)
        elif sr == WIT:
            errors[WIT] = round(
                wer(text, preprocess_transcription(transcriptions[WIT])), 2)
        elif sr == WAV2LETTER:
            errors[WAV2LETTER] = round(
                wer(text, preprocess_transcription(transcriptions[WAV2LETTER])), 2)
        elif sr == PADDLEDEEPSPEECH:
            errors[PADDLEDEEPSPEECH] = round(
                wer(text, preprocess_transcription(transcriptions[PADDLEDEEPSPEECH])), 2)
    ...
    while x < 3:
        ...
        while (not q.empty() and last_time < time_max):
            ...
            if (is_predicted_bug):
                ...
                errors = calculate_recognition_error(
                    data["text"], transcriptions)
                bugs = {}
